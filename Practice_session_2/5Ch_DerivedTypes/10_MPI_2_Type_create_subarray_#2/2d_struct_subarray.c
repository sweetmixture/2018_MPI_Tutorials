...
*/
    ...
    /* MPI_Type_create_subarray Practice */
    ...
    const int ndims=2;
    const int array_of_size[] = {3,3};  // original 3x3
    const int array_of_subsize[] = {2,2};   // subsize 2x2
    const int array_of_start[] = {1,0};
    // order = MPI_ORDER_C  ... internal MPI Handle

    MPI_Type_create_subarray(ndims,array_of_size,array_of_subsize,array_of_start,MPI_ORDER_C,MPI_DOUBLE,&newtype);
    MPI_Type_commit(&newtype);
    
    MPI_Bcast(&ptc->h[0][0],1,newtype,MPI_Root,MPI_COMM_WORLD);
