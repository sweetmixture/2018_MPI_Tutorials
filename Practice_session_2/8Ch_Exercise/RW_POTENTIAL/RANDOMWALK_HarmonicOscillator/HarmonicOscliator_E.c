...
int main(int argc, char* argv[]){
	...
	TotalE_state = (int*)malloc(H_state*sizeof(int));
	////////////////////////////////////////////////////
...
////////////////////////TRIAL MOVE LOOP////////////////////////////////////////////////////////
		...
		for(j=0;j<N_Oscillator;j++){
			...
			if( exp(-Delta_E/Beta) > (float)rand()/RAND_MAX )//// prob point """ -exp(*~ """";;
				Oscillator[j]++;
		...
		if( i%(int)pow(2,chk_time) == 0 ){
			sprintf(Buffer,"Distribution_Energy_%d",chk_time);
		...
		MAX_Hstate = 0;
		for(j=0;j<N_Oscillator;j++){
			TotalE_state[ Oscillator[j] ] += Oscillator[j];
